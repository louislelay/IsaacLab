...
class FrankaJengaEnv(DirectRLEnv):
    """Environment class for the Franka robot reaching task."""

    cfg: FrankaJengaEnvCfg

    def __init__(self, cfg: FrankaJengaEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_observations(self) -> dict:
        ...
        target = self.object.data.root_pos_w[:, :3] #self.target_pos + root_pos  # Convert to local frame
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        root_pos = self._robot.data.root_state_w[:, :3]
        target = self.object.data.root_pos_w[:, :3] #self.target_pos + root_pos
        ...
        # dist_reward *= self.object.data.root_pos_w[:, 2] > 0.04
